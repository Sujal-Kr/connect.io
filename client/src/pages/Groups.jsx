...
const Groups = () => {
  ...
  const deleteGroupHandler = async () => {
    try {
      const { data } = await axios.delete(`${server}/api/v1/chat/${chatId}`, {
        withCredentials: true,
      });
      if (data.success) {
        toast.success("Destroyed Group");
        closeConfirmDeleteGroupHandler();
        navigate("/groups");
        refetch();
      }
    } catch (err) {
  ...
  const updateGroupName = async () => {
    ...
    try {
      const { data } = await axios.patch(
        ...
        { groupName: groupNameUpdatedValue },
        { withCredentials: true }
      );
      if (data.success) {
            ...
            <div>
              ...
              <div className="w-full max-w-xl my-6 flex flex-col gap-4">
                {chat?.members?.map((user) => (
                  <div
                    className="border shadow rounded-xl bg-white px-4"
                    key={user._id}
                  >
                    <UserItem
                      user={user}
