...
const useLoadChats = () => {
  ...
  const loadChats = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(`${server}/api/v1/chat/me`, {
        withCredentials: true,
      });
      // console.log(data)
      if (data.success) {
...
const useLoadMyGroups = () => {
  ...
  const loadGroups = async () => {
    ...
    setError("");
    try {
      const { data } = await axios.get(`${server}/api/v1/chat/me/groups`, {
        withCredentials: true,
      });
...
const addGroupMembers = async (chatId, members) => {
  const id = toast.loading("Adding Group Members...");
  try {
...
const removeGroupMember = async (chatId, member) => {
